...
class Rover():
    ...
    def task_instr(self, array):
        ...
        if (task == Task.SCIENCE.value):
            cmd = "gnome-terminal -x sh -c ros2 launch sc_fsm_drill drill.py; bash"
            #process = ""
            #LAUNCH
            if (instr == Instruction.LAUNCH.value):
                if(self.ROVER_STATE == Task.IDLE):
                    ...
                    #print("dis wallah")
                    #self.node.get_logger().info("feur")
                    #generate_launch_description()
                    #os.system("ros2 launch sc_fsm_drill drill.py")
                    
                    #self.process = subprocess.call(["gnome-terminal", "-x", "sh", "-c", "ros2 launch sc_fsm_drill drill.py; bash"])
                    #self.process = Popen(["gnome-terminal", "-x", "sh", "-c", "ros2 launch sc_fsm_drill drill.py; bash"], stdout=PIPE, stderr=PIPE)
                    #stdout, stderr = self.process.communicate()
                    #launch_ros.actions.Node()

                    #os.system('gnome-terminal -- bash -c "ros2 launch /home/rocknd79/Desktop/SC_Drill/drill_workspace/src/sc_fsm_drill/launch/drill.py; bash"')
                    #self.process = Popen(['gnome-terminal', '--', 'bash', '-c', './src/sc_fsm.sh; exec bash'])

                    #os.system('./src/sc_fsm.sh')

                    #self.runNode()
                    #process = subprocess.call(["gnome-terminal", "-x", "sh", "-c", "ros2 launch sc_fsm_drill drill.py; bash"])
    ...
    #def launch_my_package_launch(self,  *args, **kwargs):
        '''return launch.actions.IncludeLaunchDescription(
            launch.launch_description_source.
            launch.launch_description_sources.AnyLaunchDescriptionSource(
                '/home/rocknd79/Desktop/SC_Drill/drill_workspace/src/sc_fsm_drill/launch/drill.py',
                launch_arguments={
                    'my_arg': 'my_value'
                }.items()
           )
        ...
        cmd=['ros2', 'launch', '/home/rocknd79/Desktop/SC_Drill/drill_workspace/src/sc_fsm_drill/launch/drill.py', 'my_arg:=my_value'],
        output='screen',
        )'''

        #return launch.actions.ExecuteProcess( cmd=['ros2', 'launch', '/home/rocknd79/Desktop/SC_Drill/drill_workspace/src/sc_fsm_drill/launch/drill.py'],output='screen' )

    def runNode(self):

        ld = launch.LaunchDescription([
            self.launch_my_package_launch()
        ])
    ...
    def log_task_already_launched(self, task):
        ...
        status.message = "Can't launch %s if another task is still running!" % task
